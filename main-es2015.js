...
const _c2 = ["titletext"];
const _c3 = ["headCut"];
const _c4 = function (a0) { return { "transform": a0 }; };
const _c5 = function (a0) { return { "open-head": a0 }; };
const _c6 = function (a0, a1) { return { "top": a0, "left": a1 }; };
class AppComponent {
    constructor() {
    ...
    onMouseMove(e) {
        let o = this.eyes[0].x - e.x;
        let a = this.eyes[0].y - e.y;
        let p = this.get_point_between_points(this.eyes[0], e, 6);
        this.eyes[0].transform = `translateX(${p[0] - this.eyes[0].x}%) translateY(${p[1] - this.eyes[0].y}%)`;
        o = this.eyes[1].x - e.x;
        a = this.eyes[1].y - e.y;
        p = this.get_point_between_points(this.eyes[1], e, 6);
        this.eyes[1].transform = `translateX(${p[0] - this.eyes[1].x}%) translateY(${p[1] - this.eyes[1].y}%)`;
        const p2 = this.get_point_between_points(this.eyes[1], e, 120);
    }
    get_point_between_points(p1, p2, distance) {
        let dist_btw_points = Math.sqrt(Math.pow((p2.x - p1.x), 2) + Math.pow((p2.y - p1.y), 2));
        let distance_ratio = distance / dist_btw_points;
        let x = p1.x + distance_ratio * (p2.x - p1.x);
        let y = p1.y + distance_ratio * (p2.y - p1.y);
        return [x, y];
    }
    getHypotinuse(a, b) {
    ...
    fireLasers() {
        const svg = d3.select('.laser');
        svg.selectAll('line').remove();
        const el = this.eye1.nativeElement.getBoundingClientRect();
        this.eyes[0].y = el.y + el.height / 2;
        this.eyes[0].x = el.x + el.width / 2;
        this.eyes[0].width = el.width;
        const el2 = this.eye2.nativeElement.getBoundingClientRect();
        this.eyes[1].y = el2.y + el2.height / 2;
        this.eyes[1].x = el2.x + el2.width / 2;
        this.eyes[1].width = el.width;
        const title = d3.select('.title');
        // letters._groups[0].forEach(letter => {
        this.svg = d3.select('.laser');
        this.titleSVG = d3.select('#title-over');
        this.headCutSVG = d3.select('#head-cut').select('path')._groups[0];
        // this.svgBox = this.svg.getBBox()
        ...
        this.letters = title.selectAll('path')._groups[0];
        // this.letters.push(this.headCutSVG)
        let index = 0;
        let currentLetterTotal = 0;
        let position = 0;
        let point;
        while (index < this.letters.length) {
            ...
            if (position < currentLetterTotal) {
                ...
                let factor = 722 / 713.6533;
                let p = this.get_point_between_points(this.eyes[0], {
                    x: point.x * factor + this.svgPosition.left,
                    y: point.y * factor + this.svgPosition.top,
                }, 6);
                let eye0 = {
                    transform: `translateX(${p[0] - this.eyes[0].x}%) translateY(${p[1] - this.eyes[0].y}%)`,
                    x: point.x * factor + this.svgPosition.left,
                    y: point.y * factor + this.svgPosition.top,
                };
                ...
                let eye1 = {
                    transform: `translateX(${p[0] - this.eyes[1].x}%) translateY(${p[1] - this.eyes[1].y}%)`,
                    x: point.x * factor + this.svgPosition.left,
                    y: point.y * factor + this.svgPosition.top,
                };
                this.laserPaths.push([eye0, eye1, { x: point.x, y: point.y }]);
                position += 8;
            }
            else {
                position = 0;
        ...
        while (index < this.headCutSVG.length) {
            ...
            if (position < currentLetterTotal) {
                ...
                let factor = 283 / 357.89;
                let p = this.get_point_between_points(this.eyes[0], {
                    x: point.x * factor + this.headCutPosiion.left,
                    y: point.y * factor + this.headCutPosiion.top,
                }, 6);
                let eye0 = {
                    transform: `translateX(${p[0] - this.eyes[0].x}%) translateY(${p[1] - this.eyes[0].y}%)`,
                    x: point.x * factor + this.headCutPosiion.left,
    ...
    initGraphics() {
        ...
        var loadingManager = new THREE.LoadingManager(() => {
            ...
            this.balloon.position.z = 0;
            this.balloon.position.y = -0.16;
            this.scene.add(this.balloon);
        });
        const geometry = new THREE.PlaneGeometry(0.912, 0.852, 1);
        ...
        loader.load('https://assets.codepen.io/227435/Balloon.dae', (collada) => {
            collada.scene.scale.set(0.005, 0.005, 0.005);
            this.balloon = collada.scene;
        });
    ...
    createObjects() {
        ...
        if (sheetMass > 0) {
            this.rigidBodies.push(this.sheet);
            // Disable deactivation
            body.setActivationState(4);
        }
        this.physicsWorld.addRigidBody(body);
        //----------
        this.sheet.userData.physicsBody.setFriction(0.5);
        // The rope
        // Rope graphic object
        ...
        // Glue the rope extremes to the sheet and the basket
        var influence = 1;
        ...
        window.addEventListener('keydown', (event) => {
            ...
            switch (keyCode) {
                case 87: //W: FORWARD
                    ...
                    this.move.z.active = true;
                    this.move.z.direction = -1;
                    break;
                case 83: //S: BACK
    ...
    animate() {
        ...
        if (this.intro > 0 && this.letterIndex > this.laserPaths.length) {
            ...
            let resultantImpulse = new Ammo.btVector3(this.balloon.position.x, this.balloon.position.y, this.balloon.position.z);
            let scalingFactor = 42;
            resultantImpulse.op_mul(scalingFactor);
            let physicsBody = this.basket.userData.physicsBody;
        ...
        if (this.letterIndex < this.laserPaths.length) {
            ...
            this.svg
                ...
                .attr('x1', this.eyes[0].x)
                .attr('y1', this.eyes[0].y)
                .attr('x2', this.laserPaths[this.letterIndex][0].x)
                .attr('y2', this.laserPaths[this.letterIndex][0].y);
            this.svg
                .append('line')
                .attr('filter', 'url(#f1)')
                .style('stroke-width', 3)
                .attr('x1', this.eyes[1].x)
                .attr('y1', this.eyes[1].y)
                .attr('x2', this.laserPaths[this.letterIndex][1].x)
                .attr('y2', this.laserPaths[this.letterIndex][1].y);
            this.titleSVG
                .select('defs')
                .select('mask')
                .append('circle')
                .attr('cx', this.laserPaths[this.letterIndex][2].x)
                .attr('cy', this.laserPaths[this.letterIndex][2].y)
                .attr('r', 8);
    ...
    render() {
        ...
        // left and right think this is working
        ...
        if (this.move.z.active) {
            ...
            if (this.balloon) {
                if (this.balloon.rotation.x + rads(90) < -0.04 ||
                    this.balloon.rotation.x + rads(90) > 0.04) {
...
AppComponent.ɵcmp = _angular_core__WEBPACK_IMPORTED_MODULE_0__["ɵɵdefineComponent"]({ type: AppComponent, selectors: [["app-root"]], viewQuery: function AppComponent_Query(rf, ctx) { if (rf & 1) {
        ...
        _angular_core__WEBPACK_IMPORTED_MODULE_0__["ɵɵlistener"]("click", function AppComponent_Template_img_click_11_listener() { return ctx.openHead = !ctx.openHead; });
        _angular_core__WEBPACK_IMPORTED_MODULE_0__["ɵɵelementEnd"]();
...
/*@__PURE__*/ (function () { _angular_core__WEBPACK_IMPORTED_MODULE_0__["ɵsetClassMetadata"](AppComponent, [{
        ...
        args: [{
            ...
            args: ['eye1', { static: false }]
        }], eye2: [{
            type: _angular_core__WEBPACK_IMPORTED_MODULE_0__["ViewChild"],
            args: ['eye2', { static: false }]
        }], titletext: [{
            type: _angular_core__WEBPACK_IMPORTED_MODULE_0__["ViewChild"],
            args: ['titletext', { static: false }]
        }], headCut: [{
            type: _angular_core__WEBPACK_IMPORTED_MODULE_0__["ViewChild"],
            args: ['headCut', { static: false }]
        }], onMouseMove: [{
...
module.exports = __webpack_require__(/*! /Users/hwhite1/Repos/hPortfolio/src/main.ts */"./src/main.ts");
